...
- The authorized Flex resume completed all twelve automated cycles at `$0.1363645` total attempt cost. Human review still rejected Rowan chapter 2, Elara chapter 1, and Lucan chapter 1 for uncommitted mana, destination overshoot, route reversal, and private-plan reassignment. Automated audit approval is evidence, not human release approval.
- ADR-016 adds narrow state-claim validation before audit and inside the atomic commit. The exact report scan catches only those three rejected chapters. Authenticated `--rerun-from` suffix recovery keeps seven accepted chapters, reruns five, persists the decision, and preserves all prior attempts and spend. Their prior accepted cost fits current headroom by `$0.007879325`; five full chapter ceilings do not, so the durable ledger may stop a retrying run early.
...
- ADR-018 adds a narrative-only replacement path for the three human-rejected cells. It parses and exactly restages authenticated state, action, intents, delta, and frame before budget creation; runs no agents, frame generation, state writer, or store commit; keeps dependent chapter 2 results; and binds source and replacement identities around one canonical hash. Narration, recovery, and the independent 64-output-token audit use reasoning `none`. A `$0.0135` target cap gives a hard three-target maximum of `$0.0405`, projected exposure `$2.999776675`, and headroom `$0.000223325`.
...
- The authorized re-narration chain corrected Rowan chapter 2 but did not commit Elara or reach Lucan. The latest Elara attempt stopped safely before an unaffordable request. Durable exposure is `$2.993429175`; only `$0.006570825` remains, with no lock or active or uncertain reservation. No later paid command has run.
- The latest strict failure receipt is registered through exact bridge hashes. Provider-free preparation accepts only Elara chapter 1 and Lucan chapter 1. ADR-020 adds a read-only preflight and an exact one-way `$3` to `$3.021` migration after baseline synchronization. Their `$0.027` hard ceiling projects `$3.020429175` and `$0.000570825` final headroom. The unchanged `$3` ledger and provider remain untouched until fresh user authority.
- Test-first evidence hardening rejects the exact escaped Elara route contradiction and rejects native adapter labels without one accepted response containing a matched hosted spawn/output plus root final message. A retry-mixing regression and a 62-path resume regression failed before their fixes. Runtime and eval share one DRY evidence validator; independent re-review found no remaining P0 through P2 defect.
...
Phase 6 is ready for human testing. The centered Reader, meaningful-decision continuation, truthful stop and retry behavior, chapter-100 boundary, provider-free seeded evidence, six-POV sample pack, desktop and mobile browser QA, full non-live gate, and clean-clone gate passed. Phase 5 correction preflight is provider-free and exact. Work stops at fresh authority for its `$0.027` paid ceiling, then later separate native-smoke authority. User release gates remain unchanged.
