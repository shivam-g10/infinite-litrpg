...
- The latest strict failure receipt is registered through 43 exact bridge hashes. Provider-free preparation accepts only Elara chapter 1 and Lucan chapter 1. ADR-020 adds a read-only preflight and an exact one-way `$3` to `$3.021` migration after baseline synchronization. Their `$0.027` hard ceiling projects `$3.020429175` and `$0.000570825` final headroom. The exact preflight passed with byte-identical ledger and evidence files; the `$3` ledger and provider remain untouched until fresh user authority.
