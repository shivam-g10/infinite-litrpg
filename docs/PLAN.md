...
Phase 5 release eval: prompt `1.4.11` locally preserves exact failed candidates, grounds audit rejections, stops after same-prose audit exhaustion, aligns recovery, disables implicit cache writes, and has a provider-counted `$0.207336` twelve-cycle projection. Report version 8 roots committed output in reproducible state, request, response, stream, and ledger evidence. Finish full non-live gates, independent review, clean commit, and clean-clone verification. Then run the fresh sequential twelve-cycle matrix at the existing `$0.0424` chapter cap under the durable `$3` ledger.
