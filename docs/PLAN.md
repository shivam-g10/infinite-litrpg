...
Phase 5 release eval: prompt `1.4.11` locally preserves exact failed candidates, grounds audit rejections, stops after same-prose audit exhaustion, aligns recovery, disables implicit cache writes, and has a provider-counted `$0.207336` twelve-cycle projection. Report version 8 roots committed output in reproducible state, request, response, stream, and ledger evidence. Full non-live gates, independent review, commit `e4a1c77`, and clean-clone verification are green. The durable ledger has zero lock, zero active or uncertain reservations, exact exposure `$2.786385175`, and headroom `$0.213614825`. Run the fresh sequential twelve-cycle matrix exactly once at the existing `$0.0424` chapter cap.
