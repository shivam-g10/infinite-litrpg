...
- The latest strict failure receipt is registered through 40 exact bridge hashes. Provider-free preparation accepts only Elara chapter 1 and Lucan chapter 1. Their `$0.027` hard ceiling projects `$3.020429175`; the unchanged `$3` ledger blocks it. A one-way `$3.021` cap migration and exact paid ceiling need new user authority before implementation or provider access.
