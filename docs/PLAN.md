...
- Live report version 4 preserves attempt phase, deterministic rejection issues, audit evidence, approved prose, prior spend, projected exposure, and exact cumulative spend. Future paid commands pass recorded prior spend so worst-case ceilings stay under `$3`.
...
Phase 5 release eval: checkpoint prompt `1.4.5`, then run twelve complete cycles across all six viewpoints at `$0.0648` per chapter. Conservative prior exposure is `$2.222336225`; worst-case cumulative exposure is `$2.999936225`.
