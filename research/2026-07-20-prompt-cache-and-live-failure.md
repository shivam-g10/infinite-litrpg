...
- Same-model exact JSON pair counts show a minimum structural saving of 48 output tokens across all eight background actions and one output token per frame. At `$1.25` per million conservatively reserved ordinary input tokens and `$6` per million output tokens, background and frame compaction saves at least `$0.016396`.
- The adjusted twelve-cycle projection is `$0.207336`, leaving `$0.006278825` under `$0.213614825`. This excludes additional narration and audit prompt savings. Deterministic chapter 1 and chapter 2 probes found Rowan had the largest combined narration-plus-audit input among all six POVs.
